...
class Simulation:
    ...
    def run(self, episode, epsilon):
        ...
        while self._step < self._max_steps:
            """
            now we have to change the lights in a particular order
            """
        ...
        print("Training...")
        start_time = timeit.default_timer()
        for k in range(self._training_epochs):
            self._replay()
            if k != 0 and k % 100 == 0:
    ...
    def _replay(self):
        ...
        if len(batch) > 0:  # if the memory is full enough
            ...
            q_s_a_d = self._Target_Model.predict_batch(next_states)  # predict Q(next_state), for every sample
